...
if [[ "${SKIP_SSL,,}" == "y" ]]; then
    ...
    certbot certonly --standalone --non-interactive --agree-tos \
        --email "${CERT_EMAIL}" -d "${DOMAIN}" 2>/dev/null || {
        warn "Let's Encrypt failed. Is A record for ${DOMAIN} pointing to ${PUBLIC_IP}?"
...
echo -e "${GREEN}${BOLD}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "  ${BOLD}Your Dashboard (one place for everything):${NC}"
...
echo -e "    ${BLUE}curl ${PROTO}://${DOMAIN}/health${NC}"
echo ""
echo -e "  ${BOLD}What to do now:${NC}"
echo -e "    1. Go to ${CYAN}${PROTO}://${DOMAIN}${NC}"
echo -e "    2. Log in with password above"
echo -e "    3. Click ${BOLD}Providers${NC} → add your accounts:"
echo -e "       • ${CYAN}Connect Claude Code${NC}  (OAuth — click and login)"
echo -e "       • ${CYAN}Connect Gemini CLI${NC}   (OAuth — click and login)"
echo -e "       • ${CYAN}Connect Antigravity${NC}  (OAuth — click and login)"
echo -e "       • ${CYAN}Connect GitHub Copilot${NC} (OAuth — click and login)"
echo -e "       • ${CYAN}Connect Kiro${NC}          (OAuth — click and login)"
echo -e "       • ${CYAN}Connect Qwen Code${NC}     (OAuth — click and login)"
echo -e "       • ${CYAN}Connect Kimi Coding${NC}   (OAuth — click and login)"
echo -e "       • ${CYAN}Add DeepSeek API Key${NC}"
echo -e "       • ${CYAN}Add Groq API Key${NC}"
echo -e "       • ${CYAN}Add any other provider${NC}"
echo -e "    4. Go to ${BOLD}Routing${NC} → set up fallback chains"
echo -e "    5. Go to ${BOLD}API Keys${NC} → create keys for your users"
echo ""
