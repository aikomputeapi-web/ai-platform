...
function ProviderBadge({ alias }: { alias: string }) {
  return <span className="badge">{alias}</span>;
}
    ...
    <div className="border-default bg-surface mb-8">
      ...
      >
        <div className="flex-center gap-8" style={{ minWidth: 0 }}>
          ...
          <code className="mono text-13 font-700 text-bright truncate">
            {entry.id}
          </code>
        </div>
        <div className="flex-center gap-8" style={{ flexShrink: 0 }}>
          <span className="text-11 text-muted mono">
            {entry.providers.length} provider
      ...
      {expanded && (
        <div style={{ padding: "0 16px 16px 16px" }}>
          ...
          >
            <div
              className="mono text-10 uppercase text-muted mb-8 font-600"
              ...
              {entry.providers.map((p, idx) => (
                ...
                >
                  <div className="flex-center gap-12">
                    <span
                      className="mono text-11 text-muted font-700"
                      style={{ width: "16px", textAlign: "center" }}
                    >
                      {idx + 1}
                    </span>
                    <ProviderBadge alias={p.alias} />
                    <code className="mono text-11 text-muted">
                      {p.prefixedId}
                    </code>
                  </div>
                  {idx === 0 && (
...
export default function CatalogAdminTab() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (result.created > 0) {
        setSuccessMessage(
          `Catalog generated: ${result.created} models created, ${result.deleted} old entries cleaned up.`,
        );
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        `Virtual catalog ${result.enabled ? "enabled" : "disabled"}.`,
      );
    } catch {
      setError("Network error");
    } finally {
  ...
  const filteredQuarantined = useMemo(() => {
    ...
    if (!search.trim()) return data.quarantined;
    const q = search.toLowerCase();
    return data.quarantined.filter(
      (entry) =>
  ...
  if (loading || !data) {
    return (
      <div
        className="flex-center justify-center bg-bg"
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="dash-page-header flex-start flex-wrap gap-20 justify-between">
          <div>
            ...
            <p className="dash-page-sub">
              The virtual catalog replaces raw provider-prefixed lists with a
              clean, routed view.
            </p>
          </div>
        ...
        {/* Alerts */}
        {error && <div className="alert-error mb-24">{error}</div>}
        {warnings.length > 0 && (
        ...
        {successMessage && (
          <div className="alert-success mb-24">{successMessage}</div>
        )}
        ...
        <div className="dash-stats-grid">
          <div className="dash-stat">
            <div className="dash-stat-label">
              ...
              <span
                style={{
                  color: data.enabled ? "var(--accent)" : "var(--muted)",
                }}
              >
                ●
              </span>
            ...
            <div className="dash-stat-sub">
              {data.enabled
                ? "Clean /v1/models active"
                : "Raw provider names exposed"}
            </div>
          </div>
          <div className="dash-stat">
          ...
          <div className="dash-stat">
            ...
            <div className="dash-stat-sub">
              {uniqueProviders.slice(0, 3).join(", ") || "none"}
            </div>
          </div>
          <div className="dash-stat">
            <div className="dash-stat-label">Quarantined</div>
            <div className="dash-stat-value">{data.totalQuarantined || 0}</div>
            <div className="dash-stat-sub">
              omitted non-text or internal entries
            </div>
          </div>
        </div>
        ...
        <div className="dash-grid-2">
          ...
          <div className="dash-card mb-0">
            <div className="dash-card-title flex-between">
              ...
              <span
                className="badge font-mono-brand"
                style={{ fontSize: "9px" }}
              >
            ...
            <p className="text-13 text-muted mb-16">
              Each model below appears once in the customer&apos;s{" "}
              <code
                className="mono"
                style={{
                  fontSize: "11px",
            ...
            >
              {filtered.length > 0 ? (
                ...
                <p
                  className="text-13 text-muted text-center"
            ...
            {data.quarantined?.length > 0 && (
              <div
                className="border-default bg-bg"
                style={{ marginTop: "20px", padding: "16px" }}
              >
                <div className="flex-between mb-12">
                  <div>
                    <div className="font-700 text-12 text-bright">
                      Quarantined Provider Models
                    </div>
                    <p
                      className="text-11 text-muted"
                      style={{ lineHeight: "1.4", marginTop: "4px" }}
                    >
                      Omitted from the customer catalog because they are not
                      text generation models or are operational aliases.
                    </p>
                  </div>
                  <span className="badge">{filteredQuarantined.length}</span>
                </div>
                <div
                  className="flex flex-col gap-6"
                  style={{ maxHeight: "280px", overflowY: "auto" }}
                >
                  {filteredQuarantined.map((entry) => (
                    <div
                      key={entry.id}
                      className="border-default bg-surface flex-between gap-12"
                      style={{ padding: "8px 10px" }}
                    >
                      <div style={{ minWidth: 0 }}>
                        <code className="mono text-11 text-bright block truncate">
                          {entry.id}
                        </code>
                        <span className="text-10 text-muted mono">
                  ...
                  {filteredQuarantined.length === 0 && (
                    <p
                      className="text-11 text-muted text-center"
                      style={{ padding: "16px 0" }}
                    >
                      No quarantined entries match this search.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="dash-card mb-0">
              ...
              <div className="flex flex-col gap-12">
                <div className="border-default p-12 bg-bg">
                  <div className="font-700 text-12 text-bright mb-4">
                    1. Clean Model List
                  </div>
                  <p
                    className="text-11 text-muted"
                  ...
                  >
                    Customers calling{" "}
                    <code
                      className="mono text-10 bg-surface"
                      style={{ padding: "1px 3px" }}
                    >
                      GET /v1/models
                    </code>{" "}
                    see each model once without provider prefixes.
                  </p>
                </div>
                <div className="border-default p-12 bg-bg">
                  <div className="font-700 text-12 text-bright mb-4">
                  ...
                  >
                    ...
                    , the system tries providers in priority order until one
                    succeeds.
                  </p>
                </div>
                <div className="border-default p-12 bg-bg">
                  ...
                  <p
                    ...
                    style={{ lineHeight: "1.4" }}
                  >
                    Customers don&apos;t need to change anything. Their existing
                    API calls work — they just see cleaner model names.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-6">
                {uniqueProviders.length > 0 ? (
                  ...
                  <p className="text-11 text-muted">
                    No providers detected. Generate the catalog first.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="dash-card mb-0">
              ...
              <div className="flex flex-col gap-8">
                ...
                >
                  ...
                  {data.enabled
                    ? "Disable virtual catalog"
                    : "Enable virtual catalog"}
                </button>
              </div>
            </div>
          </div>
